...
import { Calendar, BarChart3, Table2, Cog, RefreshCw, Radio, Clock, Users, Zap } from "lucide-react";

const AutomationBackground = () => {
  const [syncPulse, setSyncPulse] = useState(false);
  const [liveValue, setLiveValue] = useState(847);
  const [machineStatus, setMachineStatus] = useState([true, true, false, true, true, false, true, true]);
    ...
    <div className="absolute inset-0 bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
        <svg className="w-full h-full">
          <defs>
            <pattern id="automation-grid" width="40" height="40" patternUnits="userSpaceOnUse">
      ...
      <div className={`absolute inset-0 transition-opacity duration-500 ${syncPulse ? 'opacity-5' : 'opacity-0'}`}>
      ...
      <div className="absolute top-[8%] left-[4%] flex gap-2">
        {[
          ...
          <div key={i} className="bg-white/5 backdrop-blur-sm rounded-lg border border-white/10 p-2 w-20">
      ...
      <div className="absolute top-[8%] right-[4%] flex items-center gap-2">
        <RefreshCw className={`w-4 h-4 text-emerald-400 ${syncPulse ? 'animate-spin' : ''}`} />
      ...
      <div className="absolute top-[22%] left-[4%] w-[28%] bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-3">
        ...
        <div className="space-y-1">
          {tableRows.map((row, i) => (
            <div key={i} className="flex items-center justify-between text-[9px] py-1 border-b border-white/5">
      ...
      <div className="absolute top-[22%] left-[35%] w-[30%] bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-3">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white/70 text-[10px] font-medium">LIVE METRICS</span>
          </div>
      ...
      <div className="absolute top-[22%] right-[4%] w-[24%] bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-white/70 text-[10px] font-medium">SCHEDULE</span>
        </div>
          ...
          {calendarDays.map((d, i) => (
            <div
              ...
              className={`aspect-square rounded flex items-center justify-center text-[8px] relative ${
                d.isToday ? 'bg-indigo-500 text-white' :
                d.hasEvent ? 'bg-white/10 text-white/80' : 'text-white/40'
              }`}
            >
              {d.day}
      ...
      <div className="absolute bottom-[25%] left-[4%] w-[28%] bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-white/70 text-[10px] font-medium">MACHINES</span>
        </div>
        <div className="grid grid-cols-4 gap-2">
          {machineStatus.map((active, i) => (
            ...
            >
              <div className={`w-2 h-2 rounded-full ${active ? 'bg-emerald-400' : 'bg-red-400 animate-pulse'}`} />
            </div>
          ))}
        </div>
        <div className="flex justify-between mt-2 text-[8px]">
          <span className="text-emerald-400">● {machineStatus.filter(Boolean).length} Online</span>
          <span className="text-red-400">● {machineStatus.filter(s => !s).length} Offline</span>
        </div>
      </div>
      ...
      <div className="absolute bottom-[25%] left-[35%] w-[30%] bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-3">
        <div className="flex items-center justify-between mb-2">
          <span className="text-white/70 text-[10px] font-medium">SHIFT PROGRESS</span>
      ...
      <div className="absolute bottom-[25%] right-[4%] w-[24%] space-y-2">
        {["API Gateway", "Database", "Cache", "Queue"].map((service, i) => (
          <div key={i} className="flex items-center gap-2 bg-white/5 rounded-lg px-2 py-1.5 border border-white/10">
            <div className={`w-1.5 h-1.5 rounded-full ${i === 2 ? 'bg-amber-400' : 'bg-emerald-400'} animate-pulse`} />
            ...
            <div className="flex gap-0.5">
              {[...Array(5)].map((_, j) => (
                <div
                  ...
                  style={{ animationDelay: `${j * 100}ms`, height: `${4 + Math.random() * 8}px` }}
                />
              ))}
            </div>
          </div>
      ...
      <div className="absolute bottom-10 left-6 md:left-12">
        ...
        <p className="text-white/60 text-sm mt-1">Everything. Connected. Live.</p>
      </div>
