...
import { Plug, Webhook, Cloud, Database, Zap, RefreshCw, CheckCircle, AlertCircle, Settings } from "lucide-react";
...
const IntegrationBackground = () => {
  ...
  const platforms = [
    { name: "Salesforce", x: 18, y: 22, color: "from-blue-400/30 to-blue-600/30", status: "connected" },
    { name: "Slack", x: 38, y: 15, color: "from-purple-400/30 to-purple-600/30", status: "connected" },
    { name: "AWS", x: 58, y: 24, color: "from-amber-400/30 to-amber-600/30", status: "syncing" },
    { name: "Stripe", x: 78, y: 18, color: "from-violet-400/30 to-violet-600/30", status: "connected" },
    { name: "GitHub", x: 13, y: 52, color: "from-gray-400/30 to-gray-600/30", status: "connected" },
    { name: "Jira", x: 33, y: 60, color: "from-blue-400/30 to-blue-600/30", status: "error" },
    { name: "Twilio", x: 55, y: 55, color: "from-red-400/30 to-red-600/30", status: "connected" },
    { name: "Zapier", x: 73, y: 50, color: "from-orange-400/30 to-orange-600/30", status: "connected" },
  ];
  ...
  const recentSyncs = [
    { app: "Salesforce", action: "Contact sync", records: 247, status: "success" },
  ...
  return (
    <div className="absolute inset-0 bg-gradient-to-br from-[hsl(var(--gradient-4-start))] to-[hsl(var(--gradient-4-end))] overflow-hidden">
      ...
      <div className="absolute top-[6%] left-[4%] flex gap-3">
        {[
          { label: "API Calls", value: apiCalls.toLocaleString(), icon: Zap, color: "text-emerald-400" },
          { label: "Connected", value: "24", icon: Plug, color: "text-cyan-400" },
          { label: "Webhooks", value: "18", icon: Webhook, color: "text-violet-400" },
        ].map((stat, i) => (
      ...
      {platforms.map((platform, i) => (
        ...
        >
          <div
            className={`w-full h-full rounded-lg bg-gradient-to-br ${platform.color} backdrop-blur-md border border-white/20 flex flex-col items-center justify-center ${
              connections.includes(i) ? "shadow-[0_0_25px_rgba(255,255,255,0.3)]" : ""
            }`}
          >
            <span className="text-white/80 text-[8px] font-medium">{platform.name}</span>
            <div className={`w-1.5 h-1.5 rounded-full mt-1 ${
              platform.status === 'connected' ? 'bg-emerald-400' :
              platform.status === 'syncing' ? 'bg-cyan-400 animate-pulse' :
              'bg-red-400'
            }`} />
          </div>
        </div>
      ...
      <div className="absolute top-[6%] right-[4%] w-[26%] bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-3">
        <div className="flex items-center gap-2 mb-2">
          <RefreshCw className={`w-3 h-3 text-white/60 ${activeSync % 2 === 0 ? 'animate-spin' : ''}`} />
          <span className="text-white/70 text-[10px] font-medium">RECENT SYNCS</span>
        </div>
      ...
      {/* Webhook Monitor */}
      <div className="absolute bottom-[28%] right-[4%] w-[26%] bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-3">
        <div className="flex items-center gap-2 mb-2">
          <Webhook className="w-3 h-3 text-white/60" />
          <span className="text-white/70 text-[10px] font-medium">WEBHOOKS</span>
        </div>
        <div className="space-y-1.5">
          {webhooks.map((webhook, i) => (
            <div key={i} className="flex items-center gap-2 text-[8px]">
              <div className={`w-1.5 h-1.5 rounded-full ${
                webhook.status === 'active' ? 'bg-emerald-400' : 'bg-amber-400'
              }`} />
              <span className="text-white/50 font-mono flex-1 truncate">{webhook.endpoint}</span>
      ...
      {/* Integration Health */}
      <div className="absolute bottom-[28%] left-[4%] w-[24%] bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-3">
        <div className="flex items-center gap-2 mb-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div key={i} className="text-center p-1.5 bg-white/5 rounded-lg">
