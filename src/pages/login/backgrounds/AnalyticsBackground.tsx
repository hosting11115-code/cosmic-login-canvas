...
const AnalyticsBackground = () => {
  const [chartData, setChartData] = useState<number[]>([
    40, 65, 45, 80, 55, 90, 70, 85, 90, 70, 85,
  ]);
  ...
  useEffect(() => {
    const dataInterval = setInterval(() => {
      setChartData((prev) =>
        prev.map((v) =>
          Math.max(20, Math.min(95, v + (Math.random() - 0.5) * 20))
        )
      );
    }, 2000);
    ...
    const conversionInterval = setInterval(() => {
      setConversionRate((prev) =>
        Math.max(5, Math.min(15, prev + (Math.random() - 0.5) * 0.5))
      );
    }, 2000);
  ...
  const hourlyData = [
    12, 8, 5, 3, 4, 8, 15, 28, 45, 52, 48, 55, 62, 58, 54, 48, 52, 65, 72, 68,
    55, 42, 28, 18,
  ];
  ...
  return (
    <div className="absolute inset-0 bg-gradient-to-br from-[hsl(var(--gradient-5-start))] to-[hsl(var(--gradient-5-end))] overflow-hidden">
      ...
      {/* Main chart */}
      <div className="absolute left-[4%] top-[18%] w-[36%] h-[30%]">
        <div className="w-full h-full bg-gray-700/40 backdrop-blur-sm rounded-xl border border-white/10 p-3">
          ...
          {/* Bar chart */}
          <div className="flex items-end justify-between h-[calc(100%-50px)] gap-4 px-1">
            {chartData.map((value, i) => (
              ...
              >
                <div
                  className={cn(
                    "w-full bg-gradient-to-t from-transparent  rounded-t transition-all duration-1000",
                    i % 2 === 0 ? "to-white/10" : "to-white/30"
                  )}
                  style={{ height: `${value}%` }}
                />
      ...
      <div className="absolute left-[4%] bottom-[30%] w-[36%] bg-gray-700/40 backdrop-blur-sm rounded-xl border border-white/10 p-3">
      ...
      {/* Metrics cards */}
      <div className="absolute right-[4%] top-[10%] space-y-2 w-[26%]">
        {metrics.map((metric, i) => {
          const Icon = metric.icon;
          return (
            <div
              key={i}
              className={`p-3 bg-gray-700/40 backdrop-blur-sm rounded-xl border transition-all duration-500 ${
                activeMetric === i
      ...
      {/* Traffic Sources */}
      <div className="absolute right-[6%] bottom-[12%] w-[26%] bg-gray-700/40 backdrop-blur-sm rounded-xl border border-white/10 p-3">
        <div className="flex items-center gap-2 mb-2">
        ...
        <div className="space-y-1.5">
          {trafficSources.map((item, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <span className="text-white/40 text-[8px] w-6 text-right">
                {item.value}%
              </span>
            </div>
          ))}
      ...
      {/* Top Pages */}
      <div className="absolute left-[35%] bottom-[12%] w-[24%] bg-gray-700/40 backdrop-blur-sm rounded-xl border border-white/10 p-3">
        ...
        <div className="space-y-1 mt-2">
          {topPages.map((page, i) => (
            ...
            >
              ...
              <span className="text-white/70 font-mono flex-1 truncate">
                {page.page}
              </span>
      ...
      {/* Floating stats */}
      <div className="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 animate-float">
        <div className="absolute inset-0 animate-ripple rounded-full border-2 border-white/20 opacity-0" />
        <div className="absolute inset-0 animate-ripple-delayed-1 rounded-full border-2 border-white/15 opacity-0" />
        <div className="absolute inset-0 animate-ripple-delayed-2 rounded-full border-2 border-white/10 opacity-0" />
        <div className="absolute inset-0 animate-ripple-delayed-3 rounded-full border-2 border-white/5 opacity-0" />
        <div className="w-20 h-20 rounded-full bg-gray-700/40 backdrop-blur-md border border-white/10 flex flex-col items-center justify-center">
